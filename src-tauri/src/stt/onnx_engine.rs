    ...
    pub fn initialize(path: &Path, _use_gpu: bool) -> Result<Self, AppError> {
        ...
        match detect_onnx_layout(dir) {
            OnnxLayout::Transducer { encoder, decoder, joiner } => {
                ...
                // Modified beam search over the transducer lattice recovers accuracy
                // on harder audio at a small cost compared with greedy decoding.
                config.decoding_method = Some("modified_beam_search".to_string());
                config.max_active_paths = 4;
                // NeMo Parakeet needs an explicit model type. k2/icefall Zipformer
                // models ship `bpe.model` and use sherpa's default transducer type.
                if !dir.join("bpe.model").exists() {
